...
### Structure of a GASM Program

In GASM, like most assembly style languages, a program is a list of instructions.
...
while others take one, two, or three. Its common that the first operand is the destination.

Comments start with a semicolon (`;`). Everything after the semicolon on a line is ignored
by when pasting into GRAB.
...
Before we dive into writing programs with GASM, lets break down a simple
program.

Below is a program written in GASM.

```gasm
; infinitely move 'Obj' upwards

SET R0 10                      ; speed to move

LABEL loop                     ; label to jump back to
	ADD Obj.Pos.Y Obj.Pos.Y R0 ; add the speed to the position
	SLEEP 0                    ; sleep till next frame
...
Line 6 adds `R0` (which is 10) to `Obj.Pos.Y` and put the result into
...
Its important te remember that this program is equivelant to the following, but
...
The primary way to store and manipulate data in GASM is through
**working registers**. GASM provides eight working registers,
named `R0` through `R7`.
...
When you add a connection you can connect the position, rotation, or active state,
and can name the connection.
...
Position and rotation are readable and writable and active is readonly
...
> NOTE:
rotations are calculated in X Y Z order. This will probably not effect you, but may come
in handy if you know what it means.
...
Arithmatic operators take three arguments and set the first to the result of the second and the third.
...
Control flow allow for jumping around the code to create
...
This code is equivelant to the previous.
...
##### SLEEP
`SLEEP` pauses execution for a given amount of time in ms.
...
> As of writing this, ProgramCounter is not accessable.

##### DeltaTime

DeltaTime stores the time since the last frame. It should be used to make moving objects
smooth and consistent by multiplying movment by the DeltaTime to ensure the framerate doesnt
effect the speed.
...
It is important to also include a `SLEEP 0` at the end of the loop to ensure the loop only runs once per frame.
...
Generally when moving something, you want it to be at  constant rate.
...
But if you are lagging, then the object will move slower because there are less frames.

The way to counteract this to make the movement appear smooth, and be consistent locally and across multiplayer is to multiply the movemenbt by DeltaTime.
...
The JSON Editor has a simple but very powerful extra tool on topp of GASM.
...
#END
```
This code uses `#FOR` to duplicate code that sets the `X`, `Y` , and `Z` of an object to a random value `-10` through `10`.
The for loop creates a variable `n` at `1` and loops up to `3`. Step is omitted so it is 1.
For each loop it sets `Obj#n.Pos.X` where `#n` will be replaced with the current value of `n`. e.g., `Obj1.Pos.X`.

There is also an `#IF` that runs when `n == 3`. This means each loop will check that condition and if it is true,
will add the `SET R0 10` line which forces the ojects `X` position to be `0`.

`IF` and `FOR` variable arguments must be alphanumeric.
`IF` and `FOR` value arguments must be either a number, an existing variable, or a single operator expression such as `i*j`.
`IF` operators can be `==`, `<`, `<=`, `>`, `>=`, or `!=`.

The resulting GASM code will set 3 objects to random -10 - 10 positions but object 3's X will always be 0:
