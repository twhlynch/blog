...
Recently, GRAB has added code blocks, allowing users to build programs with a block
...
going into detail on the different parts of the language.
...
Comments start with a semicolon (`;`). Everything after the semicolon on a line is removed
...
Comments are highly recommended to explain longer programs, they make them
...
The main way to store and manipulate data in GRAB Assembly is through
...
These are simple operators that move data in registers.
...
> NOTE: As of writing this, ProgramCounter is not yet accessible.
