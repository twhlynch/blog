...
Before we dive into writing programs with GASM, let's break down a simple
program.
...
It's important to remember that this program is equivelant to the following, but
...
> NOTE:
Rotations are calculated in X -> Y -> Z order. This means when the game computes
how an object should be rotated based on the rotation values, the object is
first rotated around the X axis, then the result of that rotation is rotated
around the Y axis, and finally around the Z axis, so changing one rotation
...
The JSON Editor has a simple but very powerful extra tool on top of GASM.
...
#END
```
This code uses `#FOR` to duplicate code that sets the `X`, `Y`, and `Z`
of an object to a random value `-10` through `10`.
...
will add the `SET R0 10` line, which forces the oject's `X` position to be `0`.
