...
Recently, GRAB has added code blocks, which allow users to build programs with a block
coding interface for writing **assembly**. This post introduces GRAB Assembly
from a more technical point of view: writing code to copy into
the game, and using the JSON Editor to write complex programs.

I first explain the structure of GRAB assembly, and explain a simple program before
going into detail on the different parts of the language: registers, connections,
operators, some commonly used patterns, and using macros.
...
> NOTE: Operands can be referred to as 'Operands' to an operator, or 'Arguments' to an instruction.
...
while others take one, two, or three. Most commonly, the first operand is the destination.
...
Line 1, as well as the text after the semicolon (;) on each line, is a comment. It will be removed
when pasting into GRAB, but is handy to explain what code is doing.
...
By using comments and indenting, this program becomes more easily readable,
but in GRAB, is equivalent to the following:
...
Registers have no fixed meaning and their purpose is entirely
...
and to add comments that describe how each register is currently being used.

If you need more than 8 registers, you can use connections as extra places to store data.
However, you should be aware that the object exists physically.

### Connections

> Connections can be added in GRAB's selection menu. Select a code block,
> click `Edit Connections`, and select another object to add a connection to it.

When you add a connection to an object, you can connect the position, rotation,
...
**Position** and **rotation** are readable and writable. **Active** is read-only
and only available for connections to triggers.

Connections can be used anywhere a register can, except for `Act`
connections which cannot be modified in code.

> NOTE:
> Rotations are calculated in Y -> X -> Z order. This means that, when the game computes
> how an object should be rotated based on the rotation values: first, the object is
> rotated around the Y axis; then the result of that rotation is rotated
> around the X axis; and finally the result of that rotation is rotated around the
> Z axis. So changing one rotation axis can affect how subsequent rotations behave.
...
The `SET` operator takes two arguments: a destination register (`DST`), and a source value (`SRC`).
...
The `SWAP` operator takes two register arguments (`A`, and `B`), and swaps their values.
...
Arithmetic operators take three arguments: a destination register (`DST`), and two register or value arguments (`A`, and `B`).
...
The `FLOOR` operator takes two arguments: a destination register (`DST`), and a source value (`SRC`).
...
The `SQRT` operator takes two arguments: a destination register (`DST`), and a source value (`SRC`).
...
The `SIN` operator takes two arguments: a destination register (`DST`), and a source angle (`SRC`).
It puts the sine of the source's value as an angle into the destination.
...
The `COS` operator takes two arguments: a destination register (`DST`), and a source angle (`SRC`).
It puts the cosine of the source's value as an angle into the destination.
...
Because the dot always stays on the circle, these values
...
import TrigDiagram from "@components/TrigDiagram.astro";

<TrigDiagram />

##### ATAN2

The `ATAN2` operator takes two arguments: a destination register (`DST`),
...
```gasm
ATAN2 R0 0 1 ; set R0 to atan2 of 0 and 1
; R0 is 90
```
...
While I prefer this kind of pattern of having reversed conditions,
(if not less than 0), and jumping over logic, others may prefer to have the
...
> NOTE: At the time of writing this, ProgramCounter is not yet accessible.
...
preprocessor macros that allow for generating far more complex programs.
